...
class SOLOHead(BaseMaskHead):
    ...
    def __init__(
        ...
        self.filter_center_constraint = filter_center_constraint
    ...
    def forward(self, x: Tuple[Tensor]) -> tuple:
        ...
        for i in range(self.num_levels):
            ...
            if self.centerness:
                ...
                centerness = cls_pred.new_zeros(cls_pred.size())
            ...
            mlvl_centerness_preds.append(centerness)
        return mlvl_mask_preds, mlvl_cls_preds, mlvl_centerness_preds

    def loss_by_feat(
        self,
        mlvl_mask_preds: List[Tensor],
        mlvl_cls_preds: List[Tensor],
        mlvl_centerness_preds: List[Tensor],
        ...
        **kwargs
        ...
        # positive
        pos_mask_targets, labels, pos_masks, centers = multi_apply(
        ...
        mlvl_centers = [[] for _ in range(num_levels)]
        mlvl_centers_preds = [[] for _ in range(num_levels)]
        for img_id in range(num_imgs):
            ...
            for lvl in range(num_levels):
                ...
                mlvl_centers[lvl].append(centers[img_id][lvl].flatten()[pos_masks[img_id][lvl]])
                mlvl_centers_preds[lvl].append(mlvl_centerness_preds[lvl][img_id].flatten()[pos_masks[img_id][lvl]])
        ...
        for lvl in range(num_levels):
            ...
            mlvl_centers[lvl] = torch.cat(mlvl_centers[lvl], dim=0)
            mlvl_centers_preds[lvl] = torch.cat(mlvl_centers_preds[lvl], dim=0)
        ...
        flatten_centerness_preds = torch.cat(mlvl_centers_preds)
        flatten_centers = torch.cat(mlvl_centers)
        loss_centerness = self.loss_centerness(
            flatten_centerness_preds, flatten_centers, avg_factor=num_pos + 1
        )
        if self.centerness:
            return dict(loss_mask=loss_mask, loss_cls=loss_cls, loss_centerness=loss_centerness)
        else:
    ...
    def _get_targets_single(
        ...
        for (lower_bound, upper_bound), stride, featmap_size, num_grid in zip(
            ...
            centers = torch.zeros([num_grid, num_grid], device=device)
            ...
            for gt_mask, gt_label, pos_h_range, pos_w_range, valid_mask_flag, gt_box in zip(
                hit_gt_masks,
                hit_gt_labels,
                pos_h_ranges,
                pos_w_ranges,
                ...
                for i in range(top, down + 1):
                    for j in range(left, right + 1):
                        ...
                        pos_mask[index] = True
                        center_h, center_w = center_of_mass(gt_mask)
                        center_h_dist = (i * gt_mask.size(0)/num_grid + gt_mask.size(0)/(2*num_grid) - center_h)
                        center_w_dist = (j * gt_mask.size(1)/num_grid + gt_mask.size(1)/(2*num_grid) - center_w)
                        center_dist = ((center_h_dist)**2 + (center_w_dist)**2).sqrt()
                        normalizer = ((pos_h_range**2 + pos_w_range**2).sqrt())/(self.pos_scale*output_stride)
                        centerness = max(torch.tensor(0, device=normalizer.device), 1 - (center_dist / normalizer))
                        centers[i, j] = max(centerness, centers[i, j])

            mlvl_pos_mask_targets.append(mask_target[pos_mask])
            mlvl_labels.append(labels)
            mlvl_pos_masks.append(pos_mask)
            mlvl_centers.append(centers)
        return mlvl_pos_mask_targets, mlvl_labels, mlvl_pos_masks, mlvl_centers

    def predict_by_feat(
        self,
        mlvl_mask_preds: List[Tensor],
        mlvl_cls_scores: List[Tensor],
        mlvl_centerness: List[Tensor],
        batch_img_metas: List[dict],
        **kwargs
    ) -> InstanceList:
        ...
        mlvl_centerness = [item.permute(0, 2, 3, 1).squeeze(-1) for item in mlvl_centerness]
    ...
    def _predict_by_feat_single(
        ...
        centers_scores = centers_scores[inds[:, 0]]
        ...
        centers_scores = centers_scores[keep]
        inds = inds[keep]

        if self.filter_center_constraint:
            keep_centers = cls_scores.new_zeros(cls_scores.size(0)).bool()
            for i in range(len(cls_scores)):
                lvl = (inds[i, 0] < lvl_interval).nonzero()[0, 0]
                pos = inds[i, 0]
                if lvl > 0:
                    pos = pos - lvl_interval[lvl - 1]
                center_h, center_w = center_of_mass(masks[i])
                mask_center_h = floordiv(
                    center_h / masks.size(1), 1 / self.num_grids[lvl]
                )
                mask_center_w = floordiv(
                    center_w / masks.size(2), 1 / self.num_grids[lvl]
                )
                cls_center_h, cls_center_w = (
                    pos // self.num_grids[lvl],
                    pos - (pos // self.num_grids[lvl]) * self.num_grids[lvl],
                )
                ...
                w_range, h_range = width*0.5*self.pos_scale, height*0.5*self.pos_scale
                keep_centers[i] = (cls_center_h-mask_center_h).abs() <= h_range and (cls_center_w-mask_center_w).abs() <= w_range
            masks = masks[keep_centers]
            mask_preds = mask_preds[keep_centers]
            sum_masks = sum_masks[keep_centers]
            cls_scores = cls_scores[keep_centers]
            cls_labels = cls_labels[keep_centers]
            centers_scores = centers_scores[keep_centers]
